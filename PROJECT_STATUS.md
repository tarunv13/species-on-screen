...
1. **EPR vents atlas field record** — `atlas/epr-vents.html`; renders the M19 DwC-A (`public/dwca/epr-vents/`) and, in the same milestone, registers the archive in `public/dwca/index.json` (deferred in M19 so the atlas discovery chip never points at a missing page). Completes the EPR 3-surface experience.
