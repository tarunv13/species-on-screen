/*
  The Crossing — scroll-governed cinematic migration (prototype controller).
  --------------------------------------------------------------------------
  A single full-viewport canvas renders a dark ocean field across which a
  luminous route is traced as the viewer scrolls. The camera follows the
  head of the route and descends (zooms from planetary framing toward the
  water) — the Descent grammar (Article III) generalised into a journey.
  ...
  Nothing is named (platform-architecture §5). No species, no place, no
  coordinates, no counts, no sources. The fragments are observational and
  fewer than twelve words each (editorial-voice; ingestion workflow §3).
*/
...
/* ---------- Biome palette ----------
   Grounded in the authored hawksbill safari_scene block
   (public/data/hawksbill-turtle.json). Cool, oceanic, near-black. */
const PAL = {
  abyss:      [4, 7, 13],      // deepest held darkness
  deepWater:  [10, 20, 36],    // #0a1424
  midWater:   [20, 60, 92],    // derived from mid #1a5a7e, darkened
...
/* ---------- The route ----------
   ...
   These are stylised coordinates, not geography — the cinematic surface
   carries no real lat/long (platform-architecture §3). */
const FIELD_W = 2000;
const FIELD_H = 1400;
const ROUTE = [
...
(function buildLUT() {
  const pts = ROUTE;
  const SEG = 90; // samples per segment
  let prev = null;
...
function resize() {
  ...
  // Fit the whole field within the frame at planetary zoom, with margin.
  baseFit = Math.min(W / FIELD_W, H / FIELD_H);
}
window.addEventListener('resize', resize);

/* Camera state derived from progress p (0..1). */
function camera(p) {
  const follow = smoothstep(0.10, 0.42, p);            // wide → locked to head
  const zoom = lerp(0.94, 3.1, smoothstep(0.08, 0.94, p)); // the Descent
...
/* ---------- Ambient particulate (marine snow) ----------
   Screen-space drifting motes — atmosphere in front of the lens. Under
   the 1500-particle ceiling (Article V). Additive, very low alpha. They
   keep faint motion alive even when the scroll is still. */
const MOTES = [];
(function seedMotes() {
...
/* ---------- Editorial fragments ----------
   Place-before-name: nothing here names the animal or the shore. Each is
   fewer than twelve words. One subject per beat (Article IX); the gaps
   between windows are held darkness (Article VI). */
const BEATS = [
  ...
  { at: 0.955, text: 'every crossing ends where it began', kind: 'inscription' },
];
...
function beatOpacity(b, p, t) {
  if (b.kind === 'framing') {
    // Held at the very top; recedes as the first push begins.
    return (1 - smoothstep(0.05, 0.115, p)) * 0.95;
  }
  if (b.kind === 'inscription') {
    // Arrives at the Settle and breathes — never resolves to zero.
...
let amb = 0; // ambient clock (advances only when motion is allowed)

function paintBase(progress, beachP) {
  // Vertical ocean gradient: deepest at the bottom, a faint atmospheric
  // lift toward the top. The water warms a touch at arrival (pre-dawn).
  ...
  // A slow, breathing pool of faint light — the moon on the water, an
  // atmospheric presence rather than a named source (Article V).
...
function paintShore(cam, beachP) {
  ...
  // Mass: a soft pale shore glow grows in at the destination.
  ...
  // Place-specificity: a surf line that moves (sine), arriving after mass.
  ...
  // Inhabitant — last layer (Article XVII). Not the animal: a track in
  // the sand. Two short, faint dashes that arrive only at the very end.
...
function paintRoute(cam, progress) {
  // Sample the traced portion 0..progress and stroke it with a soft
  // halation core (Article V: highlight bleed, not uniform glow).
  const N = 240;
  ...
  // Origin reef glint — brightest early, dimming as we Drift away.
  ...
  // The head — a single point of light. A presence, not the named animal.
  // Brightens faintly at arrival.
...
function paintLuminanceDip(progress) {
  // The Descent's one sanctioned cut is hidden in a luminance dip
  // (Article III). Peaks at open ocean (~0.5) — the held-darkness heart,
  // and the "no shore, no star" beat. Uniform, narrative, full-frame —
  // not a corner vignette (Article V forbids the namable frame device).
...
function frame(now) {
  ...
  // Smooth the scroll into a slow push.
  p += (rawP - p) * (REDUCE ? 0.30 : 0.085);

  render();

  // Drive caption opacities (one subject per beat).
...
function init() {
  resize();
  readScroll();
  // A quiet invitation to scroll, surfacing after the threshold settles.
